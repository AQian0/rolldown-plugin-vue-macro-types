...
export const createLanguageServiceManager = (
  ...
  const getService = (id: string): ts.LanguageService => {
    ...
    const tsconfigPath =
      options.tsconfig ?? ts.findConfigFile(path.dirname(id), ts.sys.fileExists, "tsconfig.json");
    ...
    const serviceHost: ts.LanguageServiceHost = {
      ...
      readFile: (fileName) => virtualFiles.get(fileName)?.content ?? ts.sys.readFile(fileName),
    };
